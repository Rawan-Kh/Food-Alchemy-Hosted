        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              ...
              <Input id="name" value={newIngredient.name} onChange={e => setNewIngredient({
              ...newIngredient,
              name: e.target.value
            ...
            <div>
              ...
              <Input id="quantity" type="number" min="0" step="0.1" value={newIngredient.quantity} onChange={e => setNewIngredient({
              ...newIngredient,
            ...
            <div>
              ...
              <select id="unit" value={newIngredient.unit} onChange={e => setNewIngredient({
              ...newIngredient,
              unit: e.target.value
            ...
            <div>
              ...
              <select id="category" value={newIngredient.category} onChange={e => setNewIngredient({
              ...newIngredient,
            ...
            <div>
              ...
              <Input id="expiry" type="date" value={newIngredient.expiryDate} onChange={e => setNewIngredient({
              ...newIngredient,
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid grid-cols-3 lg:grid-cols-9 mb-6">
              ...
              return <TabsTrigger key={category.id} value={category.id} className="flex flex-col items-center gap-1 p-2 text-xs">
                    <Icon className="w-4 h-4" />
                    <span className="hidden sm:inline">{category.name}</span>
                    <span className="sm:hidden">{category.name.split(' ')[0]}</span>
                    {count > 0 && <Badge variant="secondary" className="text-xs">
                        {count}
                      </Badge>}
                  </TabsTrigger>;
            })}
            ...
            return <TabsContent key={category.id} value={category.id}>
                  <div className="space-y-4">
                    ...
                    {categoryIngredients.length === 0 ? <div className="text-center py-8 text-gray-500">
                    ...
                    return <div key={`ingredient-${ingredient.id}`} className="border rounded-lg p-4 space-y-2">
                              <div className="flex justify-between items-start">
                                <h4 className="font-semibold capitalize">{ingredient.name}</h4>
                                <Button variant="ghost" size="sm" onClick={() => onRemoveIngredient(ingredient.id)} className="text-red-500 hover:text-red-700">
                                  <Trash className="w-4 h-4" />
                                </Button>
                              </div>
                              
                              <div className="flex items-center gap-2">
                                <Input type="number" min="0" step="0.1" value={ingredient.quantity} onChange={e => onUpdateQuantity(ingredient.id, parseFloat(e.target.value) || 0)} className="w-20" />
