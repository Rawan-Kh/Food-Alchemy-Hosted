...
export const GuidedChecklist: React.FC<GuidedChecklistProps> = ({
  ...
  // Show checklist on first visit or when not all items are completed
  useEffect(() => {
    const hasSeenChecklist = localStorage.getItem('recipe-app-checklist-seen');
    if (!hasSeenChecklist || !isAllCompleted) {
      setIsVisible(true);
    }
  }, [isAllCompleted]);
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ChevronRight className={`w-3 h-3 transition-transform ${isMinimized ? 'rotate-90' : '-rotate-90'}`} />
            </Button>
            <Button 
              variant="ghost" 
              size="sm"
