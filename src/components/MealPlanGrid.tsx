...
export const MealPlanGrid: React.FC<MealPlanGridProps> = ({
  ...
  return <>
      <div className="grid grid-cols-8 gap-4 min-w-[768px]">
        ...
        {DAYS_OF_WEEK.map(day => <div key={day} className="font-semibold text-center py-2">
            {day.slice(0, 3)}
          </div>)}
        ...
        color
          ...
          return <Card key={`${day}-${type}`} className="min-h-[80px]">
                  <CardContent className="p-2">
                    {recipe ? <div className="space-y-2">
                        <div className="flex items-start justify-between">
                          <h4 className="text-xs font-medium line-clamp-2">
                            {recipe.name}
                          </h4>
                          <Button variant="ghost" size="sm" onClick={() => onRemoveRecipe(day, type)} className="h-6 w-6 p-0 text-red-500 hover:text-red-700">
                            <X className="h-3 w-3" />
                          </Button>
                        </div>
                      ...
                      </div> : <Button variant="ghost" className="w-full h-full flex items-center justify-center" onClick={() => setSelectedSlot({
                day,
                mealType: type
              })}>
      ...
      {selectedSlot && <RecipeSelectionDialog isOpen={true} onClose={() => setSelectedSlot(null)} onSelect={handleRecipeSelect} recipes={recipes} mealType={selectedSlot.mealType} day={selectedSlot.day} />}
    </>;
};