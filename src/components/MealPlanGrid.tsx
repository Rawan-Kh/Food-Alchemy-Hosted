...
const MEAL_TYPES: { type: MealType; label: string; color: string }[] = [
  { type: 'breakfast', label: 'Breakfast', color: 'bg-yellow-100 text-yellow-800' },
  { type: 'snack', label: 'Snack', color: 'bg-green-100 text-green-800' },
  { type: 'lunch', label: 'Lunch', color: 'bg-blue-100 text-blue-800' },
  { type: 'dinner', label: 'Dinner', color: 'bg-purple-100 text-purple-800' }
];
...
export const MealPlanGrid: React.FC<MealPlanGridProps> = ({
  ...
  const [selectedSlot, setSelectedSlot] = useState<{ day: string; mealType: MealType } | null>(null);
  ...
  return (
    <>
      <div className="grid grid-cols-8 gap-4">
        {/* Header row */}
        <div className="font-semibold text-center py-2">Meal</div>
